...
ABoard* ABoard::InstanciaBoard = nullptr;
ABoard* ABoard::GetInstanciaBoard()
{
    if (!InstanciaBoard) {
        InstanciaBoard = NewObject<ABoard>();
        GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Red, TEXT("Se creo el board"));
    }
    else {
        GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Red, TEXT("No se puede crear mas board"));
    }
    return InstanciaBoard;
}
